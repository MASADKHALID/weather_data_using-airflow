...
def save_joined_data_s3(task_instance):
    ...
    df.to_csv(f"s3://s3-pp-asad-new/{dt_string}.csv", index=False)

default_args = {
    'owner': 'airflow',
    'depends_on_past': False,
    'start_date': datetime(2023, 1, 8),
    'email': ['asadkhalidking55@gmail.com'],
...
with DAG('weather_dag_2',
        ...
        with TaskGroup(group_id = 'group_a', tooltip= "Extract_from_S3_and_weatherapi") as group_A:
            ...
            uploadS3_to_postgres  = PostgresOperator(
                ...
                postgres_conn_id = "postgres_conn",
                sql = "SELECT aws_s3.table_import_from_s3('city_look_up', '', '(format csv,DELIMITER'','',HEADER true)','s3-pp-asad','us_city.csv','us-east-1');"
            )
            ...
            is_houston_weather_api_ready = HttpSensor(
                ...
                endpoint='/data/2.5/weather?q=karachi&appid=887c2865ce5489a445a1484b7881ecf4'
            )
